Feature: Sharing
    ...
    Background:
        ...
        And the administrator on the server has set the default folder for received shares to ""
    ...
    Scenario: try to share a file/folder with a user to whom the file has already been shared
        ...
        And user "Alice" has uploaded file with content "ownCloud test text file" to "/textfile.txt" on the server
        And user "Alice" has shared file "SharedFolder" on the server with user "Brian" with "all" permissions
    ...
    Scenario: sharee renames the shared file and folder
        ...
        Then as "Alice" folder "FOLDER" should not exist on the server
